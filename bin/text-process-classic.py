...
# Vendored change (Cam)
# - Inline the `setText` macro definition, rather than including `Tool Helpers`
...
RE_TEXT_ENTRY = re.compile(
    r"^#\s*([0x[0-9a-fA-F]+|#)\s*(\w+)?\s*(({?[*^])|})?$", re.I
)  # added third group for checking narrow
...
class Preprocessor:
    ...
    def preprocess(self, fileName):
        if fileName in self.includeSet:
            sys.stderr.write(
                "WARNING: file `{}` was already included once, ignoring.\n".format(
                    fileName
                )
            )
            return None
                    ...
                    if directive == "include":
                        ...
                        if not m2:
                            raise TextProcessError(
                                fileName,
                                iLine + 1,
                                "Bad define! Replacement string is probably missing.",
                            )
                        ...
                        sys.stderr.write(
                            'WARNING: {}:{}: What is a "#{}"? is this is comment? consider using "//" for comments instead!\n'.format(
    ...
    for fileName, iLine, line in lines:
        ...
        if currentText == None:  # no current text, reading entry header
            if l == "":
                ...
                if not match:
                    raise TextProcessError(
                        fileName, iLine + 1, "expected entry header!"
                    )
            ...
            if l[-3:] == "[X]":  # Line ends in [X] (end of text entry)
                result.append(
                    TextEntry(currentText, currentStringId, currentDefinition)
                )

                if doTrace:
                    sys.stderr.write(
                        "TRACE: [generate_text_entries] read {}\n".format(
                            result[-1].get_pretty_identifier()
                        )
                    )
...
def generate_text_binary(
    parseFileExe, parseDefinitions, textEntry, sourceFile, targetFile
):
...
def main(args):
    ...
    argParse.add_argument(
        "--installer",
        default="Install Text Data.event",
        help="name of the installer event file to produce",
    )
    argParse.add_argument(
        "--definitions",
        default="Text Definitions.event",
        help="name of the definitions event file to produce",
    )
    argParse.add_argument(
        "--parser-exe", default=None, help="name/path of the parser executable"
    )
    argParse.add_argument(
        "--depends", default=[], nargs="*", help="files that text depends on"
    )
    argParse.add_argument(
        "--parse-definitions",
        default=None,
        help="path to ParseFile defintions (implies --depends)",
    )
    argParse.add_argument(
        "--force-refresh",
        action="store_true",
        help="pass to forcefully refresh generated files",
    )
    argParse.add_argument(
        "--verbose", action="store_true", help="print processing details to stdout"
    )
    ...
    if arguments.depends:
        timeThreshold = max(
            [os.path.getmtime(filename) for filename in arguments.depends]
        )
    ...
    try:
        ...
        for entry in generate_text_entries(
            Preprocessor(verbose).preprocess(inputPath), verbose
        ):  # create separate files for each text entry
            if entry.stringId in usedStringIds:
                sys.stderr.write(
                    "WARNING: Duplicate entry for text Id {:03X}! (ignoring)\n".format(
                        entry.stringId
                    )
                )
                ...
                if entry.definition:
                    sys.stderr.write(
                        "NOTE: Second entry was defined as `{}`\n".format(
                            entry.definition
                        )
                    )
            ...
            if entry.definition and (entry.definition in usedDefinitions):
                sys.stderr.write(
                    "WARNING: Duplicate entry definition {}! (ignoring)\n".format(
                        entry.definition
                    )
                )
    ...
    try:
        with open(outputPath, "w") as f:
            ...
            f.write(TOOL_HELPERS)
            f.write(
                '#include "{}"\n\n'.format(
                    os.path.relpath(outputDefPath, os.path.dirname(outputPath))
                )
            )
            ...
            for entry in entryList:
                textFileName = os.path.join(
                    textFolder,
                    "{}{}.fetxt".format(inputName, entry.get_unique_identifier()),
                )
                ...
                if hasParser:
                    if (
                        not os.path.exists(dataFileName)
                        or textNeedsUpdate
                        or os.path.getmtime(dataFileName) < textModifyTime
                    ):
                        if verbose:
                            sys.stderr.write(
                                "TRACE: [write] update `{}`\n".format(dataFileName)
                        ...
                        generate_text_binary(
                            parserExePath,
                            arguments.parse_definitions,
                            entry,
                            textFileName,
                            dataFileName,
                        )
                ...
                if hasParser:
                    f.write(
                        '#incbin "{}"\n'.format(
                            os.path.relpath(dataFileName, os.path.dirname(outputPath))
                        )
                    )
                else:
                    f.write(
                        '#incext ParseFile "{}"\n'.format(
                            os.path.relpath(textFileName, os.path.dirname(outputPath))
                        )
                    )
        ...
        sys.exit(
            "ERROR: ParseFile errored while parsing text for {}:\n  {}".format(
                e.textEntry.get_pretty_identifier(), e.errDesc
            )
        )
