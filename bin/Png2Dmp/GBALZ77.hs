...
import Data.Word
import Prelude hiding (drop, length, take, takeWhile, zip)
  ...
  where
    ...
    makeTypesByte =
        foldl
            ( \acc ->
                ( \e -> case snd e of
                    Literal x -> acc
                    Compressed x y -> acc .|. shift 1 (7 - fst e)
                )
            )
            0
            . zip (fromList [0 .. 7])
    makeData acc dat =
        acc >< case dat of
            ...
            Compressed lookBehind len ->
                let
                    encodedLookbehind = (lookBehind - 1)
                    encodedLength = (len - 3)
                 in
                    singleton (fromIntegral (shift (encodedLookbehind) (-8)) .|. shift (fromIntegral encodedLength) 4) |> (fromIntegral (encodedLookbehind .&. 0xFF))
...
getMaxMatch lst maxMatchLength baseOffs offsets = foldl1 (\acc -> (\e -> if snd e >= snd acc then e else acc)) $ Prelude.zip offsets $ takeWhileInclusive (< maxMatchLength) $ map (\otherOffs -> matchLength lst baseOffs otherOffs maxMatchLength {- (flip (matchLength lst baseOffs) maxMatchLength) -}) offsets
...
matchLength lst off1 off2 maxMatchLength =
    if {-# SCC matchLength_check #-} lst ! off1 /= lst ! off2
        then 0
