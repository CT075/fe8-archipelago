...
import FEParser (parseDefinitions, parseFileWithDefinitions)
import FlagUtilities
import GBAUtilities (makeError, stripExtension)
...
main = do
        ...
        then putStr $ makeError toStdOut "Usage: ./ParseFile <filename.txt> [-o <outputfile.dmp>] [-defs <definitions_file.txt>] [--to-stdout] [--q] [--help]"
            ...
            if length params /= 1
                then putStr $ makeError toStdOut "Incorrect number of parameters. Use ./ParseFile --help for usage."
                    ...
                    if elem "-o" args && getParamAfterFlag "-o" args == Nothing
                        then putStr $ makeError toStdOut "No output file specified."
                        else
                            if elem defsFlag args && getParamAfterFlag defsFlag args == Nothing
                                then putStr $ makeError toStdOut "No definitions file specified."
                                else do return ()
    let inputFileName = head params

    let definitionsFilename =
            if elem defsFlag args {- TODO: This is a bit of a lazy hack, it should be taken care of above tbqh -}
    ...
    if toStdOut
        then case parsedFile of
            ...
            if quiet
                then
                    let contents = (case parsedFile of Left result -> result; Right err -> pack $ makeError True ("Error parsing file " ++ inputFileName ++ ", line " ++ show (fst err) ++ ": " ++ (snd err)))
